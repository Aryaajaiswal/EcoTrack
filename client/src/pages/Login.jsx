    ...
    <div className="min-h-screen hero-bg flex flex-col justify-start sm:justify-center items-center px-4 py-12 sm:py-20 relative overflow-hidden">
      ...
      <motion.div initial={{ opacity:0,y:30 }} animate={{ opacity:1,y:0 }} transition={{ duration:0.5 }} className="w-full max-w-md relative z-10">
        <div className="text-center mb-8">
          ...
          <p className="text-gray-400 text-sm mt-1.5">Sign in to continue your eco-journey</p>
        </div>
        <div className="glass-strong rounded-2xl p-6 sm:p-10 shadow-2xl">
          <form onSubmit={handleSubmit} className="flex flex-col gap-5">
            ...
            <button type="submit" disabled={loading}
              className="w-full flex items-center justify-center gap-2 py-3.5 bg-emerald-500 hover:bg-emerald-400 disabled:bg-emerald-800 disabled:cursor-not-allowed text-black font-bold rounded-xl transition-all glow-emerald-sm cursor-pointer mt-2">
          ...
          <div className="mt-6 text-center pt-2">
            <p className="text-gray-400 text-sm">Don't have an account? <Link to="/register" className="text-emerald-400 font-bold hover:underline transition-colors">Create one free</Link></p>
          </div>
        </div>
      </motion.div>
    </div>
