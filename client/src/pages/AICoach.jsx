        ...
        <div className="flex gap-2 mb-6">
          {[{id:'chat',label:'💬 Chat'},{id:'predictions',label:'📈 Predictions'},{id:'tips',label:'💡 Daily Tips'}].map(t=>(
            <button key={t.id} onClick={()=>setTab(t.id)}
              className={`px-4 py-2 rounded-xl text-sm font-medium transition-all ${tab===t.id?'bg-emerald-500 text-black':'glass-card text-gray-400 hover:text-emerald-400'}`}>
              {t.label}
            </button>
          ))}
        </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
            ...
            <div className="space-y-3">
              ...
              {QUICK_PROMPTS.map((p,i)=>(
                <button key={i} onClick={()=>send(p)} className="w-full text-left p-3 glass-card rounded-xl text-gray-300 hover:text-emerald-400 border border-gray-700/50 hover:border-emerald-500/30 text-xs transition-all">
                  {p}
                </button>
              ))}
            </div>
