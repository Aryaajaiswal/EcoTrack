...
const Num = ({ label, value, onChange, min=0, max, step=1, unit }) => (
  <div>
    <div className="flex justify-between items-center mb-1.5">
      ...
      {unit && <span className="text-emerald-400 text-xs font-semibold bg-emerald-500/10 px-2 py-0.5 rounded-md border border-emerald-500/20">{unit}</span>}
    </div>
...
export default function Calculator() {
      ...
      <div className="pt-16 min-h-screen dashboard-bg">
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-10 lg:py-16">
          <motion.div initial={{opacity:0,scale:.95}} animate={{opacity:1,scale:1}} className="space-y-6 sm:space-y-8">
            ...
            <button onClick={()=>{setResult(null);setStep(0)}} className="w-full py-3 glass-card border border-emerald-500/30 hover:border-emerald-500 text-emerald-400 font-semibold rounded-xl transition-all">
              Recalculate
            </button>
          </motion.div>
        </div>
  ...
  return (
    <div className="pt-24 pb-12 min-h-screen dashboard-bg flex flex-col items-center justify-start">
