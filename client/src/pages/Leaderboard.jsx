        ...
        <div className="flex gap-2 justify-center mb-6 flex-wrap">
          {[{v:'xp',l:'⚡ By XP'},{v:'streak',l:'🔥 By Streak'},{v:'carbon_saved',l:'🌿 By CO₂ Saved'}].map(o=>(
            <button key={o.v} onClick={()=>setSortBy(o.v)}
              className={`px-4 py-2 rounded-xl text-sm font-medium transition-all ${sortBy===o.v?'bg-emerald-500 text-black':'glass-card text-gray-400 hover:text-emerald-400'}`}>
              {o.l}
            </button>
          ))}
        </div>
              ...
              <motion.div key={e.user_id} initial={{opacity:0,x:-20}} animate={{opacity:1,x:0}} transition={{delay:i*.03}}
                ...
                <div className="text-right shrink-0">
                  ...
                  <p className="text-gray-500 text-xs">
                    {sortBy === 'carbon_saved' ? `${e.xp.toLocaleString()} XP` : `${formatCO2(e.total_carbon_saved)} saved`}
                  </p>
                </div>
              </motion.div>
            ))}
