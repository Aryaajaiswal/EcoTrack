...
import { Leaf, Mail, Lock, Eye, EyeOff, User, AlertCircle, CheckCircle, Info } from 'lucide-react'
...
export default function Register() {
  ...
  const handleSubmit = async e => {
    ...
    try {
      ...
      authExtraAPI.resendVerification(form.email).catch(() => {})
  ...
  return (
    <div className="min-h-screen hero-bg flex flex-col justify-start sm:justify-center items-center px-4 py-12 sm:py-20 relative overflow-hidden">
      ...
      <motion.div initial={{ opacity:0,y:30 }} animate={{ opacity:1,y:0 }} transition={{ duration:0.5 }} className="w-full max-w-md relative z-10">
        <div className="text-center mb-8">
          <Link to="/" className="inline-flex items-center gap-2 mb-4 group">
            <div className="w-10 h-10 rounded-xl bg-emerald-500/20 flex items-center justify-center transition-transform group-hover:scale-105 duration-350"><Leaf className="w-5 h-5 text-emerald-400"/></div>
          ...
          <p className="text-gray-400 text-sm mt-1.5">Create your free account and start tracking</p>
        </div>
        <div className="glass-strong rounded-2xl p-6 sm:p-10 shadow-2xl">
          <form onSubmit={handleSubmit} className="flex flex-col gap-5">
              {error && (
                <motion.div initial={{ opacity:0,x:-10 }} animate={{ opacity:1,x:0 }}
                  className="flex items-center gap-2 p-3.5 rounded-xl bg-red-500/10 border border-red-500/20 text-red-400 text-sm">
                  <AlertCircle className="w-4 h-4 shrink-0"/>{error}
                </motion.div>
            )}
            {showVerifyMsg && (
              <motion.div initial={{ opacity:0,x:-10 }} animate={{ opacity:1,x:0 }}
                className="flex items-center gap-2 p-3.5 rounded-xl bg-blue-500/10 border border-blue-500/20 text-blue-400 text-sm">
                <Info className="w-4 h-4 shrink-0"/><span>Account created! Check the server console for your verification link (dev mode).</span>
              </motion.div>
            )}
            {[
              { key:'name', type:'text', icon:User, placeholder:'Your name', label:'Full Name' },
              { key:'email', type:'email', icon:Mail, placeholder:'your@email.com', label:'Email Address' },
            ].map(f=>(
              <div key={f.key}>
                ...
                <div className="relative">
                  ...
                  <input type={f.type} value={form[f.key]} onChange={e=>setForm(p=>({...p,[f.key]:e.target.value}))} placeholder={f.placeholder}
                    className="w-full input-with-icon pr-4 py-3 bg-gray-800/50 border border-gray-700 focus:border-emerald-500 rounded-xl text-white placeholder-gray-500 text-sm transition-colors"
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input type={showPass?'text':'password'} value={form.password} onChange={e=>setForm(p=>({...p,password:e.target.value}))} placeholder="Min. 6 characters"
                  className="w-full input-with-icon pr-10 py-3 bg-gray-800/50 border border-gray-700 focus:border-emerald-500 rounded-xl text-white placeholder-gray-500 text-sm transition-colors"
                  style={{ paddingLeft: '2.75rem' }}/>
            ...
            <button type="submit" disabled={loading}
              className="w-full flex items-center justify-center gap-2 py-3.5 bg-emerald-500 hover:bg-emerald-400 disabled:bg-emerald-800 disabled:cursor-not-allowed text-black font-bold rounded-xl transition-all glow-emerald-sm mt-2 cursor-pointer">
          ...
          <div className="mt-6 text-center pt-2">
            <p className="text-gray-400 text-sm">Already have an account? <Link to="/login" className="text-emerald-400 font-bold hover:underline transition-colors">Sign in</Link></p>
          </div>
        </div>
