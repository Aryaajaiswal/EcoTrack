...
import { Edit3, MapPin, Save, X, Award, Flame, Zap, Leaf, TrendingDown, Download } from 'lucide-react'
            ...
            <div className="flex-1">
              <div className="flex items-start justify-between gap-4">
                ...
                <div className="flex gap-2">
                  <a href={exportAPI.carbonCSV()} target="_blank" rel="noopener noreferrer"
                    className="flex items-center gap-1.5 px-3 py-2 glass-card border border-gray-600 hover:border-emerald-500 text-gray-300 hover:text-emerald-400 rounded-xl text-xs transition-all shrink-0"
                    onClick={() => toast.success('Downloading carbon data CSV...')}>
                    <Download className="w-3.5 h-3.5"/>Export CSV
                  </a>
                  <a href={exportAPI.profileJSON()} target="_blank" rel="noopener noreferrer"
                    className="flex items-center gap-1.5 px-3 py-2 glass-card border border-gray-600 hover:border-emerald-500 text-gray-300 hover:text-emerald-400 rounded-xl text-xs transition-all shrink-0">
                    <Download className="w-3.5 h-3.5"/>JSON
                  </a>
                  <button onClick={()=>setEditing(!editing)} className="flex items-center gap-1.5 px-3 py-2 glass-card border border-gray-600 hover:border-emerald-500 text-gray-300 hover:text-emerald-400 rounded-xl text-xs transition-all shrink-0 cursor-pointer">
          ...
          {editing && (
            <motion.div initial={{opacity:0,height:0}} animate={{opacity:1,height:'auto'}} className="mt-6 pt-6 border-t border-gray-700/50 space-y-4">
              ...
              <div className="flex gap-2">
                ...
                <button onClick={()=>setEditing(false)} className="flex items-center gap-2 px-4 py-2 glass-card border border-gray-600 text-gray-300 text-sm rounded-xl">
