...
from app.services.gamification_service import (
    ...
    check_and_award_badges, XP_REWARDS, get_eco_level_from_xp, BADGES,
)
...
async def complete_challenge(body: CompleteBody, current_user: dict = Depends(get_current_user)):
    ...
    if new_badges:
        ...
        for badge_id in new_badges:
            ...
            if badge:
                await create_notification(str(current_user["_id"]), "badge_earned", "New Badge!",
                    f"You earned the '{badge['name']}' badge! {badge['icon']}", {"badge_id": badge_id})
    if ch.get("badge_reward"):
        badge = next((b for b in BADGES if b["id"] == ch["badge_reward"]), None)
        await users_col.update_one({"_id": current_user["_id"]}, {"$addToSet": {"badges": ch["badge_reward"]}})
        if badge:
            await create_notification(str(current_user["_id"]), "badge_earned", "Challenge Badge!",
                f"You earned the '{badge['name']}' badge from completing '{ch['title']}'! {badge['icon']}",
                {"badge_id": ch["badge_reward"]})
    await create_notification(str(current_user["_id"]), "challenge_complete", "Challenge Completed",
        f"You completed '{ch['title']}' and earned {xp} XP!", {"challenge_id": ch["id"], "xp_earned": xp})
