...
async def get_user_stats(current_user: dict = Depends(get_current_user)):
    ...
    # Query all activities in the last 30 days to correctly calculate weekly and monthly CO2
    recent_activities = await activities_col.find({
        "user_id": str(current_user["_id"]),
        "date": {"$gte": now - timedelta(days=30)}
    }).to_list(1000)
    
    weekly_co2 = sum(a["co2_kg"] for a in recent_activities if a.get("date") and a["date"] >= now - timedelta(days=7))
    monthly_co2 = sum(a["co2_kg"] for a in recent_activities)
    
    level_info = get_level_progress(current_user.get("xp", 0))
    all_badges = get_all_badges()
    earned_ids = set(current_user.get("badges", []))
    total_activities = await activities_col.count_documents({"user_id": str(current_user["_id"])})
