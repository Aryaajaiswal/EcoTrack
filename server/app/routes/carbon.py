...
async def calculate_footprint(data: CarbonCalculationInput, current_user: dict = Depends(get_current_user)):
    logger.info(f"Incoming carbon calculation input: {data.dict()}")
    ...
    if new_badges:
        ...
        for badge_id in new_badges:
            badge = next((b for b in __import__("app.services.gamification_service", fromlist=["BADGES"]).BADGES if b["id"] == badge_id), None)
            if badge:
